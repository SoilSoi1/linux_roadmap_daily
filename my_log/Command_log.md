...
### - `chattr [+-=][ASacdistu]` # 设置文件隐藏属性
  - `lsattr` 读取文件隐藏属性
  - \+ 增加某一个特殊参数
  - \- 移除某一个特殊参数
  - \= 设置一定，且仅有后面接的参数

  - `-A` 设置该文件属性时，若当存取此文件时，他的存取时间atime将不会修改。
  - `-S` 一般文件是异步写入磁盘，如果加上该参数，进行任何文件的修改时，该更改会自动写入磁盘中
  - ***`-a`*** 设置该参数之后，该文件只能增加数据，而不能删除或者修改数据。
  - `-c` 该参数能自动地压缩文件，读取时再解压缩（针对大文件）
  - `-d` 会被dump备份系统忽略
  - ***`-i`*** 让一个文件无法增加、删除或者修改，保证系统安全，只有root可以运行
  - `-s` 当设置该参数时，如果文件被删除，将被完全移出磁盘，无法恢复
  - `-u` 和`-s`相反，如果该参数文件被删除，则其数据仍然保存在磁盘中，适合用于救援数据
...
***查找文件***
### `which` 寻找可执行文件
- `-a` 将所有可以由PATH路径的指令找出，而不是找出首个被找到的指
### `whereis` 由一些特定的目录中寻找文件名
- `-l` 列出whereis回去查询的几个主要目录
- `-b` 只找二进制文件
- `-m` 只找munual下的路径
- `-s` 只找source来源路径
- `-u` 寻找除了-bms以外的路径
...
`locate`是在系统每日更新的数据库中进行查找，所以速度比较快，但是如果数据库未及时更新，会导致找不到所需要的文件，所以最好在查找之前进行数据库更新，***更新方法为`updatedb`***, 即update database.
